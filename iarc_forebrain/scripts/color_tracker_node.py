...
class ColorTrackerNode:
    ...
    def __init__(self, tracker):
        ...
        self.pub = rospy.Publisher("/color_target_coordinates", Point)
    ...
    def image_raw_callback(self, msg):
        ...
        try:
            ...
            target = self.tracker(frame)
            # Publish the target coordinates to a ROS topic
            if len(target>=3):
                pub.publish(target[0], target[1], target[2])
            else:
                pub.publish(target[0], target[1], 0)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 2:
        ...
        print("> rosrun iarc_main color_tracker_node.py helmet_tracker_logic")
