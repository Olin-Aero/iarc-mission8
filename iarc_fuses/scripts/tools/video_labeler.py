...
try:
    from iarc_fuses.track.DaSiamRPN.net import SiamRPNvot
    from iarc_fuses.track.DaSiamRPN.run_SiamRPN import SiamRPN_init, SiamRPN_track
    from iarc_fuses.track.DaSiamRPN.utils import get_axis_aligned_bbox, cxy_wh_2_rect, rect_2_cxy_wh
    from iarc_fuses.utils import BoxUtils, iarc_root
except ImportError:
    # ROS-Agnostic import (hopefully)
    sys.path.append('../../')
    from iarc_fuses.track.DaSiamRPN.net import SiamRPNvot
    from iarc_fuses.track.DaSiamRPN.run_SiamRPN import SiamRPN_init, SiamRPN_track
    from iarc_fuses.track.DaSiamRPN.utils import get_axis_aligned_bbox, cxy_wh_2_rect, rect_2_cxy_wh
...
class Video_Labeler():
    ...
    def __init__(self, vid_file=False, scale=.6, start_frame_num=0):
        ...
        self.net.load_state_dict(
            torch.load(join(iarc_root(), 'data', 'SiamRPNVOT.model')))
        self.net.eval().cuda()
    ...
    def select_video(self):
        ...
        filename = askopenfilename()
                                   # show an "Open" dialog box and return the
    ...
    def adaptive_cut(self, img, rect):
        ...
        # ret, img_thresh =
        # cv2.threshold(img_gray,127,255,cv2.THRESH_BINARY_INV)
    ...
    def grab_cut(self, img, rect):
        ...
        im, contours, hierarchy = cv2.findContours(
            frame_threshold, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        if len(contours) != 0:
    ...
    def optimize_rect(self, rect_xywh, im):
        ...
        rect_xywh = np.int32(np.hstack([rect_xywh[0] - width * self.rect_pad / 2., rect_xywh[
                             1] - height * self.rect_pad / 2., rect_xywh[2] + 2 * width * self.rect_pad, rect_xywh[3] + 2 * height * self.rect_pad]))
        ...
        if self.mode == "GrabCut":
            ...
            rect_xywh = self.color_cut(
                im, np.int32(rect_xywh))  # Perform color cut
    ...
    def save(self):
        print("File saved as: " + "{}_labels.p".format(
            os.path.splitext(ntpath.basename(self.vid_file))[0]))
        pickle.dump(
            self.labels, open("{}_labels.p".format(os.path.splitext(ntpath.basename(self.vid_file))[0]), "wb"))
    ...
    def start_labeling(self):
        ...
        # tracker init
        target_pos, target_sz = np.array(rect_gc[0:2]) + np.array(
            rect_gc[2:4]) / 2, np.array(rect_gc[2:4])
        ...
        while(True):
            if old_frame_num != self.frame_num:
                ...
                if len(self.labels) <= self.frame_num - self.start_frame_num:
                    state_hist.append(
                        SiamRPN_track(state_hist[-1], im, use_gpu=True))  # track)
                    state = state_hist[-1]
                    rect_ccwh = np.concatenate(
                    ...
                    state = state_hist[
                        self.frame_num - self.start_frame_num]  # Use previous frame state
                    rect_xyxy = BoxUtils.unnormalize(
                        self.labels[self.frame_num - self.start_frame_num].bounding_rect, BoxUtils.FMT_XYXY, [height, width])

                pt0, pt1 = tuple(np.asarray(rect_xyxy[:2], np.int32)), tuple(
                    np.asarray(rect_xyxy[2:], np.int32))
            ...
            if k & 0xFF == ord('d'):
                # Drawing Mode
                self.draw_bbox()
                rect_xyxy = [self.sel_rect[0][0], self.sel_rect[
                    0][1], self.sel_rect[1][0], self.sel_rect[1][1]]
                # drawn_norm =
            ...
            if len(self.labels) <= self.frame_num - self.start_frame_num:
                print("Normalized rect: ", BoxUtils.normalize(
                    rect_xyxy, BoxUtils.FMT_XYXY, [height, width]))
                self.labels.append(Annotation(self.vid_file, self.frame_num, BoxUtils.normalize(
                    rect_xyxy, BoxUtils.FMT_XYXY, [height, width]), self.redo_annotation))
                self.redo_annotation = False
            else:  # Check if the cache should be used
                if k & 0xFF == ord('d'):
                    self.labels[self.frame_num] = Annotation(self.vid_file, self.frame_num, BoxUtils.normalize(
                        rect_xyxy, BoxUtils.FMT_XYXY, [height, width]), self.redo_annotation)
                    self.redo_annotation = False
    ...
    def draw_bbox(self):
        ...
        while(True):
            ...
            if self.sel_rect != None:
                cv2.rectangle(frame_draw, tuple(self.sel_rect[0]), tuple(
                    self.sel_rect[1]), (255, 255, 255))
            if self.x_cursor is not None:
                cv2.line(frame_draw, (self.x_cursor, 0), (
                    self.x_cursor, height), (0, 0, 255))
                cv2.line(frame_draw, (0, self.y_cursor), (
                    width, self.y_cursor), (0, 0, 255))
            ...
            if self.up_point == self.down_point:
                ...
                for i in range(0, 2):
                    if self.down_point[i] > self.up_point[i]:
                        self.down_point[i], self.up_point[
                            i] = self.up_point[i], self.down_point[i]
