...
class Interactive_Threshold_Setter():
	def __init__(self):
		...
		self.threshold_content_limit = 100 # Percent of pixels contained within threshold
	...
	def run(self):
		...
		if self.source is None:
			...
			warnings.warn("Capture is not set (using default). Set using the set_capture method.")
		...
		while True:
			...
			if self.color_rect != None:
				cv2.rectangle(frame_draw, tuple(self.color_rect[0]), tuple(self.color_rect[1]), (255, 255, 255))
			...
			if self.hsv_cropped is not None:
				...
				if self.down_point != self.up_point:
					self.hsv_cropped = self.frame_HSV[self.down_point[1]:self.up_point[1], 
	...
	def plot_histogram(self, image):
		...
		self.hists = np.fromstring(self.fig.canvas.tostring_rgb(), dtype=np.uint8,
				sep='')
	...
	def create_trackbars(self):
		...
		cv2.createTrackbar(self.confidence_name, self.window_detection_name,
			80, self.high_confidence, self.on_confidence_trackbar)
		# create switch for ON/OFF functionality 
		cv2.createTrackbar(self.switch_name, self.window_detection_name,
			0,1,self.nothing)
		...
		if event == cv2.EVENT_LBUTTONUP:
			...
			if self.up_point == self.down_point:
				...
				if self.threshold_content_limit < 50:
					warnings.warn("threshold_content_limit must be greater than 50, setting to 50.")
	...
	def update_confidence(self):
		if self.hsv_cropped is not None:
			low_b =  np.percentile(self.hsv_cropped, 100 - self.threshold_content_limit, axis=(0, 1))
			high_b = np.percentile(self.hsv_cropped, self.threshold_content_limit, axis=(0, 1))
	...
	def on_confidence_trackbar(self, val):
		...
		self.update_confidence()
		cv2.setTrackbarPos(self.confidence_name, self.window_detection_name, self.threshold_content_limit)
