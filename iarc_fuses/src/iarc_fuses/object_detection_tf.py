...
from iarc_fuses.utils import draw_bbox, expand_box, crop_box, inner_box
...
class ObjectDetectorTF(object):
    ...
    def __call__(self, img):
        ...
        if self.cmap_ is not None:
            ...
            outputs['class'] = [[self.cmap_[x] if (x in self.cmap_) else x for x in xs] for xs in outputs['class']]
            outputs['class'] = np.array(outputs['class'], dtype=np.int32)
        return outputs
